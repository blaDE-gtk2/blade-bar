...
#define IS_HORIZONTAL(window) ((window)->mode == BLADE_BAR_PLUGIN_MODE_HORIZONTAL)
...
static void         bar_window_get_property                   (GObject          *object,
                                                                 guint             prop_id,
                                                                 GValue           *value,
                                                                 GParamSpec       *pspec);
static void         bar_window_set_property                   (GObject          *object,
                                                                 guint             prop_id,
                                                                 const GValue     *value,
                                                                 GParamSpec       *pspec);
static void         bar_window_finalize                       (GObject          *object);
static gboolean     bar_window_expose_event                   (GtkWidget        *widget,
                                                                 GdkEventExpose   *event);
static gboolean     bar_window_delete_event                   (GtkWidget        *widget,
                                                                 GdkEventAny      *event);
static gboolean     bar_window_enter_notify_event             (GtkWidget        *widget,
                                                                 GdkEventCrossing *event);
static gboolean     bar_window_leave_notify_event             (GtkWidget        *widget,
                                                                 GdkEventCrossing *event);
static gboolean     bar_window_drag_motion                    (GtkWidget        *widget,
                                                                 GdkDragContext   *context,
                                                                 gint              x,
                                                                 gint              y,
                                                                 guint             drag_time);
static void         bar_window_drag_leave                     (GtkWidget        *widget,
                                                                 GdkDragContext   *context,
                                                                 guint             drag_time);
static gboolean     bar_window_motion_notify_event            (GtkWidget        *widget,
                                                                 GdkEventMotion   *event);
static gboolean     bar_window_button_press_event             (GtkWidget        *widget,
                                                                 GdkEventButton   *event);
static gboolean     bar_window_button_release_event           (GtkWidget        *widget,
                                                                 GdkEventButton   *event);
static void         bar_window_grab_notify                    (GtkWidget        *widget,
                                                                 gboolean          was_grabbed);
static void         bar_window_size_request                   (GtkWidget        *widget,
                                                                 GtkRequisition   *requisition);
static void         bar_window_size_allocate                  (GtkWidget        *widget,
                                                                 GtkAllocation    *alloc);
static void         bar_window_size_allocate_set_xy           (BarWindow      *window,
                                                                 gint              window_width,
                                                                 gint              window_height,
                                                                 gint             *return_x,
                                                                 gint             *return_y);
static void         bar_window_screen_changed                 (GtkWidget        *widget,
                                                                 GdkScreen        *previous_screen);
static void         bar_window_style_set                      (GtkWidget        *widget,
                                                                 GtkStyle         *previous_style);
static void         bar_window_realize                        (GtkWidget        *widget);
static StrutsEgde   bar_window_screen_struts_edge             (BarWindow      *window);
static void         bar_window_screen_struts_set              (BarWindow      *window);
static void         bar_window_screen_update_borders          (BarWindow      *window);
static SnapPosition bar_window_snap_position                  (BarWindow      *window);
static void         bar_window_display_layout_debug           (GtkWidget        *widget);
static void         bar_window_screen_layout_changed          (GdkScreen        *screen,
                                                                 BarWindow      *window);
static void         bar_window_active_window_changed          (WnckScreen       *screen,
                                                                 WnckWindow       *previous_window,
                                                                 BarWindow      *window);
static void         bar_window_active_window_geometry_changed (WnckWindow       *active_window,
                                                                 BarWindow      *window);
static void         bar_window_active_window_state_changed    (WnckWindow       *active_window,
                                                                 WnckWindowState   changed,
                                                                 WnckWindowState   new,
                                                                 BarWindow      *window);
static void         bar_window_autohide_queue                 (BarWindow      *window,
                                                                 AutohideState     new_state);
static void         bar_window_set_autohide_behavior          (BarWindow      *window,
                                                                 AutohideBehavior  behavior);
static void         bar_window_update_autohide_window         (BarWindow      *window,
                                                                 WnckScreen       *screen,
                                                                 WnckWindow       *active_window);
static void         bar_window_menu_popup                     (BarWindow      *window,
                                                                 guint32           event_time);
static void         bar_window_plugins_update                 (BarWindow      *window,
                                                                 PluginProp        prop);
static void         bar_window_plugin_set_mode                (GtkWidget        *widget,
                                                                 gpointer          user_data);
static void         bar_window_plugin_set_size                (GtkWidget        *widget,
                                                                 gpointer          user_data);
static void         bar_window_plugin_set_nrows               (GtkWidget        *widget,
                                                                 gpointer          user_data);
static void         bar_window_plugin_set_screen_position     (GtkWidget        *widget,
                                                                 gpointer          user_data);
...
struct _BarWindowClass
{
  BarBaseWindowClass __parent__;
};

struct _BarWindow
{
  BarBaseWindow      __parent__;

  /* unique id of this bar */
  gint                 id;
  ...
  /* screen and working area of this bar */
  GdkScreen           *screen;
  GdkRectangle         area;
...
/* used for a full BladeBarWindow name in the class, but not in the code */
typedef BarWindow      BladeBarWindow;
typedef BarWindowClass BladeBarWindowClass;
...
G_DEFINE_TYPE (BladeBarWindow, bar_window, BAR_TYPE_BASE_WINDOW)



static void
bar_window_class_init (BarWindowClass *klass)
{
  GObjectClass   *gobject_class;
  GtkWidgetClass *gtkwidget_class;

  gobject_class = G_OBJECT_CLASS (klass);
  gobject_class->get_property = bar_window_get_property;
  gobject_class->set_property = bar_window_set_property;
  gobject_class->finalize = bar_window_finalize;

  gtkwidget_class = GTK_WIDGET_CLASS (klass);
  gtkwidget_class->expose_event = bar_window_expose_event;
  gtkwidget_class->delete_event = bar_window_delete_event;
  gtkwidget_class->enter_notify_event = bar_window_enter_notify_event;
  gtkwidget_class->leave_notify_event = bar_window_leave_notify_event;
  gtkwidget_class->drag_motion = bar_window_drag_motion;
  gtkwidget_class->drag_leave = bar_window_drag_leave;
  gtkwidget_class->motion_notify_event = bar_window_motion_notify_event;
  gtkwidget_class->button_press_event = bar_window_button_press_event;
  gtkwidget_class->button_release_event = bar_window_button_release_event;
  gtkwidget_class->grab_notify = bar_window_grab_notify;
  gtkwidget_class->size_request = bar_window_size_request;
  gtkwidget_class->size_allocate = bar_window_size_allocate;
  gtkwidget_class->screen_changed = bar_window_screen_changed;
  gtkwidget_class->style_set = bar_window_style_set;
  gtkwidget_class->realize = bar_window_realize;
  ...
  gtk_widget_class_install_style_property (gtkwidget_class,
                                           g_param_spec_int ("popup-delay",
                                                             NULL,
                                                             "Time before the bar will unhide on an enter event",
                                                             1, G_MAXINT,
                                                             DEFAULT_POPUP_DELAY,
                                                             BLXO_PARAM_READABLE));
  ...
  gtk_widget_class_install_style_property (gtkwidget_class,
                                           g_param_spec_int ("popdown-delay",
                                                             NULL,
                                                             "Time before the bar will hide on a leave event",
                                                             1, G_MAXINT,
...
static void
bar_window_init (BarWindow *window)
{
  window->id = -1;
  window->locked = TRUE;
  window->screen = NULL;
  ...
  window->mode = BLADE_BAR_PLUGIN_MODE_HORIZONTAL;
  window->size = 48;
  window->nrows = 1;
  window->length = 0.10;
  ...
  /* set the screen */
  bar_window_screen_changed (GTK_WIDGET (window), NULL);
}



static void
bar_window_get_property (GObject    *object,
                           guint       prop_id,
                           GValue     *value,
                           GParamSpec *pspec)
{
  BarWindow *window = BAR_WINDOW (object);
  gchar       *position;
...
static void
bar_window_set_property (GObject      *object,
                           guint         prop_id,
                           const GValue *value,
                           GParamSpec   *pspec)
{
  ...
  switch (prop_id)
      ...
      if (window->mode != val_mode)
        {
          ...
          bar_window_screen_layout_changed (window->screen, window);
        }

      /* send the new orientation and screen position to the bar plugins */
      bar_window_plugins_update (window, PLUGIN_PROP_MODE);
      bar_window_plugins_update (window, PLUGIN_PROP_SCREEN_POSITION);
      break;
    ...
    case PROP_SIZE:
      ...
      /* send the new size to the bar plugins */
      bar_window_plugins_update (window, PLUGIN_PROP_SIZE);
      break;
    ...
    case PROP_NROWS:
      ...
      /* send the new size to the bar plugins */
      bar_window_plugins_update (window, PLUGIN_PROP_NROWS);
      break;
      ...
      if (window->length != val_double)
        {
          ...
          if (update)
            bar_window_screen_update_borders (window);
    ...
    case PROP_AUTOHIDE_BEHAVIOR:
      bar_window_set_autohide_behavior (window, MIN (g_value_get_uint (value),
                                                       AUTOHIDE_BEHAVIOR_ALWAYS));
      break;
    ...
    case PROP_OUTPUT_NAME:
      ...
      if (blxo_str_is_empty (val_string))
    ...
    case PROP_POSITION:
      ...
      if (!blxo_str_is_empty (val_string)
          && sscanf (val_string, "p=%d;x=%d;y=%d", &snap_position, &x, &y) == 3)
        {
          ...
          /* send the new screen position to the bar plugins */
          bar_window_plugins_update (window, PLUGIN_PROP_SCREEN_POSITION);
        }
      else
        {
...
static void
bar_window_finalize (GObject *object)
{
  BarWindow *window = BAR_WINDOW (object);

  /* disconnect from active screen and window */
  bar_window_update_autohide_window (window, NULL, NULL);
  ...
  (*G_OBJECT_CLASS (bar_window_parent_class)->finalize) (object);
}



static gboolean
bar_window_expose_event (GtkWidget      *widget,
                           GdkEventExpose *event)
{
  BarWindow  *window = BAR_WINDOW (widget);
  cairo_t      *cr;
  GdkColor     *color;
  guint         xx, yy, i;
  gint          xs, xe, ys, ye;
  ...
  /* expose the background and borders handled in BarBaseWindow */
  (*GTK_WIDGET_CLASS (bar_window_parent_class)->expose_event) (widget, event);
  ...
  /* alpha color */
  if (BAR_BASE_WINDOW (window)->is_composited)
    alpha = MAX (0.50, BAR_BASE_WINDOW (window)->background_alpha);
...
static gboolean
bar_window_delete_event (GtkWidget   *widget,
                           GdkEventAny *event)
{
...
static gboolean
bar_window_enter_notify_event (GtkWidget        *widget,
                                 GdkEventCrossing *event)
{
  BarWindow *window = BAR_WINDOW (widget);
  ...
  return (*GTK_WIDGET_CLASS (bar_window_parent_class)->enter_notify_event) (widget, event);
}



static gboolean
bar_window_leave_notify_event (GtkWidget        *widget,
                                 GdkEventCrossing *event)
{
  BarWindow *window = BAR_WINDOW (widget);
  ...
  if (event->detail != GDK_NOTIFY_INFERIOR
    ...
    if (window->autohide_behavior == AUTOHIDE_BEHAVIOR_INTELLIGENTLY)
      bar_window_active_window_geometry_changed (window->wnck_active_window, window);
    /* otherwise just hide the bar */
    else
      bar_window_autohide_queue (window, AUTOHIDE_POPDOWN);
  }

  return (*GTK_WIDGET_CLASS (bar_window_parent_class)->leave_notify_event) (widget, event);
}



static gboolean
bar_window_drag_motion (GtkWidget      *widget,
                          GdkDragContext *context,
                          gint            x,
                          gint            y,
                          guint           drag_time)
{
  if ((*GTK_WIDGET_CLASS (bar_window_parent_class)->drag_motion) != NULL)
    (*GTK_WIDGET_CLASS (bar_window_parent_class)->drag_motion) (widget, context, x, y, drag_time);
...
static void
bar_window_drag_leave (GtkWidget      *widget,
                         GdkDragContext *context,
                         guint           drag_time)
{
  BarWindow *window = BAR_WINDOW (widget);
...
static gboolean
bar_window_motion_notify_event (GtkWidget      *widget,
                                  GdkEventMotion *event)
{
  BarWindow  *window = BAR_WINDOW (widget);
   ...
   * the bar that is currently dragged */
  ...
  window->snap_position = bar_window_snap_position (window);
...
static gboolean
bar_window_button_press_event (GtkWidget      *widget,
                                 GdkEventButton *event)
{
  BarWindow   *window = BAR_WINDOW (widget);
  GdkCursor     *cursor;
  ...
  if (event->button == 1
      ...
      && modifiers == 0)
    {
      bar_return_val_if_fail (window->grab_time == 0, FALSE);
    ...
    {
      bar_window_menu_popup (window, event->time);

      return TRUE;
    }

end:
  if (GTK_WIDGET_CLASS (bar_window_parent_class)->button_release_event != NULL)
    return (*GTK_WIDGET_CLASS (bar_window_parent_class)->button_release_event) (widget, event);
  else
    return FALSE;
}
...
static gboolean
bar_window_button_release_event (GtkWidget      *widget,
                                   GdkEventButton *event)
{
  BarWindow *window = BAR_WINDOW (widget);
  GdkDisplay  *display;
...
static void
bar_window_grab_notify (GtkWidget *widget,
                          gboolean   was_grabbed)
{
  BarWindow *window = BAR_WINDOW (widget);
  GtkWidget   *current;
  ...
  if (GTK_IS_MENU_SHELL (current))
  ...
  else if (window->autohide_grab_block == 0)
    {
      if (current != NULL)
        {
          ...
          /* filter out grab event that did not occur in the bar window,
      ...
      /* avoid hiding the bar when the window is grabbed. this
...
static void
bar_window_size_request (GtkWidget      *widget,
                           GtkRequisition *requisition)
{
  BarWindow    *window = BAR_WINDOW (widget);
  ...
  borders = bar_base_window_get_borders (BAR_BASE_WINDOW (window));
  if (BAR_HAS_FLAG (borders, BAR_BORDER_LEFT))
    extra_width++;
  if (BAR_HAS_FLAG (borders, BAR_BORDER_RIGHT))
    extra_width++;
  if (BAR_HAS_FLAG (borders, BAR_BORDER_TOP))
    extra_height++;
  if (BAR_HAS_FLAG (borders, BAR_BORDER_BOTTOM))
    extra_height++;
...
static void
bar_window_size_allocate (GtkWidget     *widget,
                            GtkAllocation *alloc)
{
  BarWindow   *window = BAR_WINDOW (widget);
  ...
  if (G_UNLIKELY (window->autohide_state == AUTOHIDE_HIDDEN
                  || window->autohide_state == AUTOHIDE_POPUP))
    {
      ...
      bar_window_size_allocate_set_xy (window, w, h, &x, &y);
      bar_base_window_move_resize (BAR_BASE_WINDOW (window->autohide_window),
                                     x, y, w, h);
    }
  else
    {
      /* update the allocation */
      bar_window_size_allocate_set_xy (window, alloc->width,
      ...
      if (window->struts_edge != STRUTS_EDGE_NONE
          && window->autohide_state == AUTOHIDE_DISABLED)
        bar_window_screen_struts_set (window);
  ...
  if (G_LIKELY (child != NULL))
    {
      ...
      /* keep space for the bar handles if not locked */
      if (!window->position_locked)
        {
          if (IS_HORIZONTAL (window))
            {
...
static void
bar_window_size_allocate_set_xy (BarWindow *window,
                                   gint         window_width,
                                   gint         window_height,
                                   gint        *return_x,
                                   gint        *return_y)
...
static void
bar_window_screen_changed (GtkWidget *widget,
                             GdkScreen *previous_screen)
{
  BarWindow *window = BAR_WINDOW (widget);
  ...
  if (G_LIKELY (GTK_WIDGET_CLASS (bar_window_parent_class)->screen_changed != NULL))
    GTK_WIDGET_CLASS (bar_window_parent_class)->screen_changed (widget, previous_screen);
  ...
  if (G_UNLIKELY (window->screen != NULL))
    g_signal_handlers_disconnect_by_func (G_OBJECT (window->screen),
        bar_window_screen_layout_changed, window);

  /* set the new screen */
  window->screen = screen;
  g_signal_connect (G_OBJECT (window->screen), "monitors-changed",
      G_CALLBACK (bar_window_screen_layout_changed), window);
  ...
  bar_window_screen_layout_changed (screen, window);

  /* update wnck screen to be used for the autohide feature */
  wnck_screen = wnck_screen_get (gdk_screen_get_number (screen));
  wnck_window = wnck_screen_get_active_window (wnck_screen);
  bar_window_update_autohide_window (window, wnck_screen, wnck_window);
}



static void
bar_window_style_set (GtkWidget *widget,
                        GtkStyle  *previous_style)
{
  BarWindow *window = BAR_WINDOW (widget);

  /* let gtk update the widget style */
  (*GTK_WIDGET_CLASS (bar_window_parent_class)->style_set) (widget, previous_style);
...
static void
bar_window_realize (GtkWidget *widget)
{
  BarWindow *window = BAR_WINDOW (widget);

  (*GTK_WIDGET_CLASS (bar_window_parent_class)->realize) (widget);
...
static StrutsEgde
bar_window_screen_struts_edge (BarWindow *window)
{
  bar_return_val_if_fail (BAR_IS_WINDOW (window), STRUTS_EDGE_NONE);
...
static void
bar_window_screen_struts_set (BarWindow *window)
{
  ...
  bar_return_if_fail (cardinal_atom != 0 && net_wm_strut_partial_atom != 0);
  bar_return_if_fail (GDK_IS_SCREEN (window->screen));
  ...
  bar_return_if_fail (GDK_IS_WINDOW (GTK_WIDGET (window)->window));
  ...
  if (bar_debug_has_domain (BAR_DEBUG_YES))
    {
      ...
      if (struts[STRUT_LEFT] != 0)
        ...
        bar_debug (BAR_DEBUG_STRUTS, "%p: unset", window);

      if (n != -1)
        {
          bar_debug (BAR_DEBUG_STRUTS,
...
static void
bar_window_screen_update_borders (BarWindow *window)
{
  BarBorders borders = BAR_BORDER_NONE;
  ...
  switch (window->snap_position)
    {
    case SNAP_POSITION_NONE:
      borders = BAR_BORDER_TOP | BAR_BORDER_BOTTOM
    ...
    case SNAP_POSITION_N:
    case SNAP_POSITION_NC:
      borders = BAR_BORDER_BOTTOM | BAR_BORDER_LEFT
                | BAR_BORDER_RIGHT;
      break;

    case SNAP_POSITION_S:
    case SNAP_POSITION_SC:
      borders = BAR_BORDER_TOP | BAR_BORDER_LEFT
                | BAR_BORDER_RIGHT;
      break;
    ...
    case SNAP_POSITION_SE:
      borders = BAR_BORDER_LEFT | BAR_BORDER_TOP;
      break;

    case SNAP_POSITION_NW:
      borders = BAR_BORDER_RIGHT | BAR_BORDER_BOTTOM;
      break;

    case SNAP_POSITION_SW:
      borders = BAR_BORDER_RIGHT | BAR_BORDER_TOP;
      break;
    }
  ...
  if (window->length == 1.00)
    {
      if (IS_HORIZONTAL (window))
        BAR_UNSET_FLAG (borders, BAR_BORDER_LEFT | BAR_BORDER_RIGHT);
      else
        BAR_UNSET_FLAG (borders, BAR_BORDER_TOP | BAR_BORDER_BOTTOM);
    }

  /* set the borders */
  bar_base_window_set_borders (BAR_BASE_WINDOW (window), borders);
}



static inline guint
bar_window_snap_edge_gravity (gint value,
                                gint start,
                                gint end)
{
  gint center;
...
static SnapPosition
bar_window_snap_position (BarWindow *window)
{
  guint         snap_horz, snap_vert;
  GdkRectangle *alloc = &window->alloc;

  /* get the snap offsets */
  snap_horz = bar_window_snap_edge_gravity (alloc->x, window->area.x,
      window->area.x + window->area.width - alloc->width);
  snap_vert = bar_window_snap_edge_gravity (alloc->y, window->area.y,
...
static void
bar_window_display_layout_debug (GtkWidget *widget)
{
  GdkDisplay   *display;
  GdkScreen    *screen;
  gint          n, n_screens;
  ...
  bar_return_if_fail (bar_debug_has_domain (BAR_DEBUG_YES));
  ...
  for (n = 0; n < n_screens; n++)
    {
      ...
      if (bar_debug_has_domain (BAR_DEBUG_DISPLAY_LAYOUT))
        {
          g_string_append_printf (str, "{comp=%s, sys=%p:%p, rgba=%p:%p}",
              BAR_DEBUG_BOOL (gdk_screen_is_composited (screen)),
  ...
  bar_debug (BAR_DEBUG_DISPLAY_LAYOUT,
               "%p: display=%s{comp=%s}, %s", widget,
               gdk_display_get_name (display),
               BAR_DEBUG_BOOL (gdk_display_supports_composite (display)),
               str->str);
...
static void
bar_window_screen_layout_changed (GdkScreen   *screen,
                                    BarWindow *window)
{
  GdkRectangle  a, b;
  ...
  bar_return_if_fail (BAR_IS_WINDOW (window));
  bar_return_if_fail (GDK_IS_SCREEN (screen));
  bar_return_if_fail (window->screen == screen);
  ...
  if (G_UNLIKELY (bar_debug_has_domain (BAR_DEBUG_YES)))
    bar_window_display_layout_debug (GTK_WIDGET (window));
  ...
  struts_edge = bar_window_screen_struts_edge (window);
  ...
  n_monitors = gdk_screen_get_n_monitors (screen);
  bar_return_if_fail (n_monitors > 0);

  bar_debug (BAR_DEBUG_POSITIONING,
               "%p: screen=%p, monitors=%d, output-name=%s, span-monitors=%s, base=%d,%d",
               window, screen,
               n_monitors, window->output_name,
               BAR_DEBUG_BOOL (window->span_monitors),
  ...
  if (window->output_name == NULL
      ...
      if (gdk_screen_get_number (screen) != screen_num)
        {
          ...
          if (gdk_display_get_n_screens (display) - 1 < screen_num)
            {
              bar_debug (BAR_DEBUG_POSITIONING,
                           "%p: screen-%d not found, hiding bar",
                           window, screen_num);
  ...
  else
    {
      if (g_strcmp0 (window->output_name, "Automatic") == 0
          || window->output_name == NULL)
        {
          /* get the monitor geometry based on the bar position */
                  ...
                  if (G_UNLIKELY (name == NULL))
                    {
                      ...
                      g_message ("An output is set on the bar window (%s), "
          ...
          if (G_UNLIKELY (monitor_num == -1))
            {
              ...
              /* hide the bar if the monitor was not found */
              if (GTK_WIDGET_VISIBLE (window))
                gtk_widget_hide (GTK_WIDGET (window));
              return;
            }
  ...
  /* set the new working area of the bar */
  window->area = a;
  bar_debug (BAR_DEBUG_POSITIONING,
...
static void
bar_window_active_window_changed (WnckScreen  *screen,
                                    WnckWindow  *previous_window,
                                    BarWindow *window)
{
  WnckWindow *active_window;
  ...
  bar_window_update_autohide_window (window, screen, active_window);
}



static void
bar_window_active_window_geometry_changed (WnckWindow  *active_window,
                                             BarWindow *window)
{
  GdkRectangle bar_area;
  GdkRectangle window_area;

  bar_return_if_fail (WNCK_IS_WINDOW (active_window));
  bar_return_if_fail (BAR_IS_WINDOW (window));
      ...
      if (wnck_window_get_window_type (active_window) != WNCK_WINDOW_DESKTOP)
        {
          ...
          /* obtain position and dimension from the bar */
          bar_window_size_allocate_set_xy (window,
                                             window->alloc.width,
                                             window->alloc.height,
                                             &bar_area.x,
                                             &bar_area.y);
          ...
          /* show/hide the bar, depending on whether the active window overlaps
           * with its coordinates */
          if (window->autohide_state != AUTOHIDE_HIDDEN)
            {
              if (gdk_rectangle_intersect (&bar_area,  &window_area, NULL))
                bar_window_autohide_queue (window, AUTOHIDE_HIDDEN);
            }
          else
            {
              if (!gdk_rectangle_intersect (&bar_area, &window_area, NULL))
                bar_window_autohide_queue (window, AUTOHIDE_VISIBLE);
            }
        }
      else
        {
          /* make the bar visible if it isn't at the moment and the active
...
static void
bar_window_active_window_state_changed (WnckWindow  *active_window,
                                             WnckWindowState changed,
                                             WnckWindowState new,
                                             BarWindow *window)
{
  ...
  if (changed & WNCK_WINDOW_STATE_SHADED)
    bar_window_active_window_geometry_changed (active_window, window);
}



static gboolean
bar_window_autohide_timeout (gpointer user_data)
{
  BarWindow *window = BAR_WINDOW (user_data);

  bar_return_val_if_fail (window->autohide_state != AUTOHIDE_DISABLED, FALSE);
  bar_return_val_if_fail (window->autohide_state != AUTOHIDE_BLOCKED, FALSE);
...
static void
bar_window_autohide_timeout_destroy (gpointer user_data)
{
  BAR_WINDOW (user_data)->autohide_timeout_id = 0;
}



static void
bar_window_autohide_queue (BarWindow   *window,
                             AutohideState  new_state)
{
  guint delay;
  ...
  if (new_state == AUTOHIDE_DISABLED || new_state == AUTOHIDE_BLOCKED)
    {
      /* queue a resize to make sure the bar is visible */
      gtk_widget_queue_resize (GTK_WIDGET (window));
    }
  else
    {
      ...
      /* start a new timeout to hide the bar */
      window->autohide_timeout_id =
        g_timeout_add_full (G_PRIORITY_LOW, delay,
                            bar_window_autohide_timeout, window,
                            bar_window_autohide_timeout_destroy);
    }
}
...
static gboolean
bar_window_autohide_drag_motion (GtkWidget        *widget,
                                   GdkDragContext   *context,
                                   gint              x,
                                   gint              y,
                                   guint             drag_time,
                                   BarWindow      *window)
{
  bar_return_val_if_fail (BAR_IS_WINDOW (window), TRUE);
  bar_return_val_if_fail (window->autohide_window == widget, TRUE);

  /* queue a popup is state is hidden */
  if (window->autohide_state == AUTOHIDE_HIDDEN)
    bar_window_autohide_queue (window, AUTOHIDE_POPUP);
...
static void
bar_window_autohide_drag_leave (GtkWidget      *widget,
                                  GdkDragContext *drag_context,
                                  guint           drag_time,
                                  BarWindow    *window)
{
  bar_return_if_fail (BAR_IS_WINDOW (window));
  bar_return_if_fail (window->autohide_window == widget);
...
static gboolean
bar_window_autohide_event (GtkWidget        *widget,
                             GdkEventCrossing *event,
                             BarWindow      *window)
{
  bar_return_val_if_fail (BAR_IS_WINDOW (window), FALSE);
  bar_return_val_if_fail (window->autohide_window == widget, FALSE);

  if (event->type == GDK_ENTER_NOTIFY)
    bar_window_autohide_queue (window, AUTOHIDE_POPUP);
  else
    bar_window_autohide_drag_leave (widget, NULL, 0, window);
...
static void
bar_window_set_autohide_behavior (BarWindow     *window,
                                    AutohideBehavior behavior)
{
  GtkWidget   *popup;
  guint        i;
  ...
  if (window->autohide_behavior != AUTOHIDE_BEHAVIOR_NEVER)
    {
      ...
      if (window->autohide_window == NULL)
        {
          /* create the window */
          bar_return_if_fail (window->autohide_window == NULL);
          popup = g_object_new (BAR_TYPE_BASE_WINDOW,
                                "type", GTK_WINDOW_TOPLEVEL,
                                "decorated", FALSE,
                                "resizable", TRUE,
                                "type-hint", GDK_WINDOW_TYPE_HINT_DOCK,
                                "gravity", GDK_GRAVITY_STATIC,
                                "name", "BladeBarWindowHidden",
                                NULL);

          /* move the window offscreen */
          bar_base_window_move_resize (BAR_BASE_WINDOW (popup),
                                         -9999, -9999, 3, 3);

          /* bind some properties to sync the two windows */
          for (i = 0; i < G_N_ELEMENTS (properties); i++)
            {
              blxo_binding_new (G_OBJECT (window), properties[i],
      ...
      if (window->autohide_behavior == AUTOHIDE_BEHAVIOR_ALWAYS)
        {
          /* start autohide by hiding the bar straight away */
          if (window->autohide_state != AUTOHIDE_HIDDEN)
            {
              bar_window_autohide_queue (window,
        ...
        {
          /* start intelligent autohide by making the bar visible initially */
          if (window->autohide_state != AUTOHIDE_VISIBLE)
            {
              bar_window_autohide_queue (window,
    ...
    {
      /* stop autohide */
      bar_window_autohide_queue (window, AUTOHIDE_DISABLED);

      /* destroy the autohide window */
      bar_return_if_fail (GTK_IS_WINDOW (window->autohide_window));
      gtk_widget_destroy (window->autohide_window);
...
static void
bar_window_update_autohide_window (BarWindow *window,
                                     WnckScreen  *screen,
                                     WnckWindow  *active_window)
{
  bar_return_if_fail (BAR_IS_WINDOW (window));
  bar_return_if_fail (screen == NULL || WNCK_IS_SCREEN (screen));
  bar_return_if_fail (active_window == NULL || WNCK_IS_WINDOW (active_window));
      ...
      if (G_LIKELY (window->wnck_screen != NULL))
        {
          g_signal_handlers_disconnect_by_func (window->wnck_screen,
              bar_window_active_window_changed, window);
        }
      ...
      if (screen != NULL)
        {
          g_signal_connect (G_OBJECT (screen), "active-window-changed",
              G_CALLBACK (bar_window_active_window_changed), window);
        }
    }
      ...
      if (G_LIKELY (window->wnck_active_window != NULL))
        {
          g_signal_handlers_disconnect_by_func (window->wnck_active_window,
              bar_window_active_window_geometry_changed, window);
          g_signal_handlers_disconnect_by_func (window->wnck_active_window,
              bar_window_active_window_state_changed, window);
        }
      ...
      if (active_window != NULL)
        {
          g_signal_connect (G_OBJECT (active_window), "geometry-changed",
              G_CALLBACK (bar_window_active_window_geometry_changed), window);
          g_signal_connect (G_OBJECT (active_window), "state-changed",
              G_CALLBACK (bar_window_active_window_state_changed), window);

          /* simulate a geometry change for immediate hiding when the new active
           * window already overlaps the bar */
          bar_window_active_window_geometry_changed (active_window, window);
        }
    }
}
...
static void
bar_window_menu_toggle_locked (GtkCheckMenuItem *item,
                                 BarWindow      *window)
{
  bar_return_if_fail (GTK_IS_CHECK_MENU_ITEM (item));
  bar_return_if_fail (BAR_IS_WINDOW (window));
...
static void
bar_window_menu_deactivate (GtkMenu     *menu,
                              BarWindow *window)
{
...
static void
bar_window_menu_popup (BarWindow *window,
                         guint32      event_time)
{
  GtkWidget *menu;
  GtkWidget *item;
  ...
  g_signal_connect (G_OBJECT (menu), "deactivate",
      G_CALLBACK (bar_window_menu_deactivate), window);

  item = gtk_image_menu_item_new_with_label (_("Bar"));
  ...
  if (!bar_window_get_locked (window))
    {
      ...
      g_signal_connect_swapped (G_OBJECT (item), "activate",
          G_CALLBACK (bar_item_dialog_show), window);
      gtk_menu_shell_append (GTK_MENU_SHELL (menu), item);
      gtk_widget_show (item);
      ...
      g_signal_connect_swapped (G_OBJECT (item), "activate",
          G_CALLBACK (bar_preferences_dialog_show), window);
      gtk_menu_shell_append (GTK_MENU_SHELL (menu), item);
      gtk_widget_show (item);
      ...
      item = gtk_check_menu_item_new_with_mnemonic (_("_Lock Bar"));
      gtk_menu_shell_append (GTK_MENU_SHELL (menu), item);
      gtk_check_menu_item_set_active (GTK_CHECK_MENU_ITEM (item),
          window->position_locked);
      g_signal_connect (G_OBJECT (item), "toggled",
          G_CALLBACK (bar_window_menu_toggle_locked), window);
      gtk_widget_show (item);
  ...
  g_signal_connect_swapped (G_OBJECT (item), "activate",
      G_CALLBACK (bar_application_logout), NULL);
  gtk_menu_shell_append (GTK_MENU_SHELL (menu), item);
  gtk_widget_show (item);
  ...
  g_signal_connect (G_OBJECT (item), "activate",
      G_CALLBACK (bar_window_menu_help), NULL);
  gtk_menu_shell_append (GTK_MENU_SHELL (menu), item);
  gtk_widget_show (item);
  ...
  g_signal_connect (G_OBJECT (item), "activate",
      G_CALLBACK (bar_dialogs_show_about), NULL);
  gtk_menu_shell_append (GTK_MENU_SHELL (menu), item);
  gtk_widget_show (item);
...
static void
bar_window_plugins_update (BarWindow *window,
                             PluginProp   prop)
{
  GtkWidget   *itembar;
  GtkCallback  func;
  ...
  switch (prop)
    ...
    case PLUGIN_PROP_MODE:
      func = bar_window_plugin_set_mode;
      break;

    case PLUGIN_PROP_SCREEN_POSITION:
      func = bar_window_plugin_set_screen_position;
      break;

    case PLUGIN_PROP_NROWS:
      func = bar_window_plugin_set_nrows;
      break;

    case PLUGIN_PROP_SIZE:
      func = bar_window_plugin_set_size;
      break;
  ...
  if (G_LIKELY (itembar != NULL))
    {
      bar_return_if_fail (GTK_IS_CONTAINER (itembar));
...
static void
bar_window_plugin_set_mode (GtkWidget *widget,
                              gpointer   user_data)
{
  bar_return_if_fail (BLADE_IS_BAR_PLUGIN_PROVIDER (widget));
  bar_return_if_fail (BAR_IS_WINDOW (user_data));

  blade_bar_plugin_provider_set_mode (BLADE_BAR_PLUGIN_PROVIDER (widget),
                                       BAR_WINDOW (user_data)->mode);
}



static void
bar_window_plugin_set_size (GtkWidget *widget,
                              gpointer   user_data)
{
  bar_return_if_fail (BLADE_IS_BAR_PLUGIN_PROVIDER (widget));
  bar_return_if_fail (BAR_IS_WINDOW (user_data));

  blade_bar_plugin_provider_set_size (BLADE_BAR_PLUGIN_PROVIDER (widget),
                                       BAR_WINDOW (user_data)->size);
}



static void
bar_window_plugin_set_nrows (GtkWidget *widget,
                               gpointer   user_data)
{
  bar_return_if_fail (BLADE_IS_BAR_PLUGIN_PROVIDER (widget));
  bar_return_if_fail (BAR_IS_WINDOW (user_data));

  blade_bar_plugin_provider_set_nrows (BLADE_BAR_PLUGIN_PROVIDER (widget),
                                        BAR_WINDOW (user_data)->nrows);
}



static void
bar_window_plugin_set_screen_position (GtkWidget *widget,
                                         gpointer   user_data)
{
  BarWindow        *window = BAR_WINDOW (user_data);
  ...
  blade_bar_plugin_provider_set_screen_position (BLADE_BAR_PLUGIN_PROVIDER (widget),
                                                  position);
}
...
GtkWidget *
bar_window_new (GdkScreen *screen,
                  gint       id)
{
  if (screen == NULL)
    screen = gdk_screen_get_default ();

  return g_object_new (BAR_TYPE_WINDOW,
                       "id", id,
                       "type", GTK_WINDOW_TOPLEVEL,
                       "decorated", FALSE,
                       "resizable", FALSE,
...
gint
bar_window_get_id (BarWindow *window)
{
  bar_return_val_if_fail (BAR_IS_WINDOW (window), -1);
  bar_return_val_if_fail (window->id > -1, -1);
  return window->id;
}
...
gboolean
bar_window_has_position (BarWindow *window)
{
  bar_return_val_if_fail (BAR_IS_WINDOW (window), FALSE);
...
void
bar_window_set_povider_info (BarWindow *window,
                               GtkWidget   *provider,
                               gboolean     moving_to_other_bar)
{
  BarBaseWindow *base_window = BAR_BASE_WINDOW (window);

  bar_return_if_fail (BAR_IS_WINDOW (window));
  bar_return_if_fail (BLADE_IS_BAR_PLUGIN_PROVIDER (provider));

  blade_bar_plugin_provider_set_locked (BLADE_BAR_PLUGIN_PROVIDER (provider),
                                         bar_window_get_locked (window));

  if (BAR_IS_PLUGIN_EXTERNAL (provider))
    {
      if (moving_to_other_bar || base_window->background_alpha < 1.0)
        {
          bar_plugin_external_set_background_alpha (BAR_PLUGIN_EXTERNAL (provider),
              base_window->background_alpha);
        }

      if (base_window->background_style == BAR_BG_STYLE_COLOR)
        {
          bar_plugin_external_set_background_color (BAR_PLUGIN_EXTERNAL (provider),
              base_window->background_color);
        }
      else if (base_window->background_style == BAR_BG_STYLE_IMAGE)
        {
          bar_plugin_external_set_background_image (BAR_PLUGIN_EXTERNAL (provider),
              base_window->background_image);
        }
      else if (moving_to_other_bar)
        {
          /* unset the background (PROVIDER_PROP_TYPE_ACTION_BACKGROUND_UNSET) */
          bar_plugin_external_set_background_color (BAR_PLUGIN_EXTERNAL (provider), NULL);
        }
    }

  bar_window_plugin_set_mode (provider, window);
  bar_window_plugin_set_screen_position (provider, window);
  bar_window_plugin_set_size (provider, window);
  bar_window_plugin_set_nrows (provider, window);
}



void
bar_window_freeze_autohide (BarWindow *window)
{
  bar_return_if_fail (BAR_IS_WINDOW (window));
  bar_return_if_fail (window->autohide_block >= 0);
  ...
  if (window->autohide_block == 1
      && window->autohide_state != AUTOHIDE_DISABLED)
    bar_window_autohide_queue (window, AUTOHIDE_BLOCKED);
}



void
bar_window_thaw_autohide (BarWindow *window)
{
  bar_return_if_fail (BAR_IS_WINDOW (window));
  bar_return_if_fail (window->autohide_block > 0);
...
void
bar_window_set_locked (BarWindow *window,
                         gboolean     locked)
{
  bar_return_if_fail (BAR_IS_WINDOW (window));
...
gboolean
bar_window_get_locked (BarWindow *window)
{
  bar_return_val_if_fail (BAR_IS_WINDOW (window), TRUE);
...
void
bar_window_focus (BarWindow *window)
{
#ifdef GDK_WINDOWING_X11
  XClientMessageEvent event;

  bar_return_if_fail (BAR_IS_WINDOW (window));
  bar_return_if_fail (GTK_WIDGET_REALIZED (window));

  /* we need a slightly custom version of the call through Gtk+ to
   * properly focus the bar when a plugin calls
  ...
  if (gdk_error_trap_pop () != 0)
    g_critical ("Failed to focus bar window");
#else
...
void
bar_window_migrate_autohide_property (BarWindow   *window,
                                        ...
                                        const gchar   *property_base)
{
  ...
  bar_return_if_fail (BAR_IS_WINDOW (window));
  bar_return_if_fail (BLCONF_IS_CHANNEL (blconf));
  bar_return_if_fail (property_base != NULL && *property_base != '\0');

  old_property = g_strdup_printf ("%s/autohide", property_base);

  /* check if we have an old "autohide" property for this bar */
  if (blconf_channel_has_property (blconf, old_property))
    {
