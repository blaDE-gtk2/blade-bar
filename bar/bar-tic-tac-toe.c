...
static void bar_tic_tac_toe_response       (GtkDialog      *dialog,
                                              gint            response_id);
static void bar_tic_tac_toe_move           (BarTicTacToe *dialog);
static void bar_tic_tac_toe_button_clicked (GtkWidget      *button,
                                              BarTicTacToe *dialog);
static void bar_tic_tac_toe_new_game       (BarTicTacToe *dialog);
...
struct _BarTicTacToeClass
{
  XfceTitledDialogClass __parent__;
};

struct _BarTicTacToe
{
...
G_DEFINE_TYPE (BarTicTacToe, bar_tic_tac_toe, XFCE_TYPE_TITLED_DIALOG)



static void
bar_tic_tac_toe_class_init (BarTicTacToeClass *klass)
{
  GtkDialogClass *gtkdialog_class;

  gtkdialog_class = GTK_DIALOG_CLASS (klass);
  gtkdialog_class->response = bar_tic_tac_toe_response;
}



static void
bar_tic_tac_toe_init (BarTicTacToe *dialog)
{
  GtkWidget *button;
  GtkWidget *table;
  GtkWidget *separator;
  ...
  for (i = 0; i < 9; i++)
    {
      ...
      g_signal_connect (G_OBJECT (button), "clicked",
          G_CALLBACK (bar_tic_tac_toe_button_clicked), dialog);
  ...
  bar_tic_tac_toe_new_game (dialog);
}



static void
bar_tic_tac_toe_response (GtkDialog *gtk_dialog,
                            gint       response_id)
{
  bar_return_if_fail (BAR_IS_TIC_TAC_TOE (gtk_dialog));

  if (response_id == GTK_RESPONSE_ACCEPT)
    {
      /* new game */
      bar_tic_tac_toe_new_game (BAR_TIC_TAC_TOE (gtk_dialog));
    }
  else
    {
...
static gint
bar_tic_tac_toe_has_winner (gint state)
{
  guint i;
...
static gint
bar_tic_tac_toe_best_opening (gint state,
...
static gint
bar_tic_tac_toe_get_random_move (gint moves)
{
  gint i;
  gint n_moves;
  gint seed;
...
static gint
bar_tic_tac_toe_get_legal_moves (gint state)
{
  gint i, moves = 0;
...
static gint
bar_tic_tac_toe_get_winner_move (gint state,
                                   gint player)
{
  gint i;
  ...
  for (i = 0; i < 9; i++)
    if ((state & (1 << (i * 2 + 1))) == 0)
      if (bar_tic_tac_toe_has_winner (state | (player << (i * 2))) != 0)
        return 1 << i;
...
static gint
bar_tic_tac_toe_get_move_rate (gint state,
                                 gint move,
                                 gint move_for,
                                 gint next_turn,
                                 gint limit,
                                 gint depth)
{
  ...
  winner = bar_tic_tac_toe_has_winner (new_state);
  ...
  legal_moves = bar_tic_tac_toe_get_legal_moves (new_state);
  for (i = 0; i < 9; i++)
    {
      if ((legal_moves & (1 << i)) != 0)
        {
          value = bar_tic_tac_toe_get_move_rate (new_state, i, move_for,
...
static gint
bar_tic_tac_toe_get_move (gint state,
                            gint player,
                            gint level)
{
  gint winner;
  ...
  winner = bar_tic_tac_toe_has_winner (state);
  if (winner == 0)
    {
      legal_moves = bar_tic_tac_toe_get_legal_moves (state);
      ...
      switch (level)
        ...
        case LEVEL_EXPERT:
          ...
          moves = bar_tic_tac_toe_best_opening (state, first_moves);
          if (moves != 0)
            break;
          ...
          for (i = 0; i < 9; i++)
            {
               ...
               value = bar_tic_tac_toe_get_move_rate (state, i, player, player, 15, 1);
               if (value > rate)
                 {
        ...
        case LEVEL_INTERMEDIATE:
          ...
          moves = bar_tic_tac_toe_get_winner_move (state, PLAYER_X);
          if (moves != 0)
            break;

          /* try to find a blocking move */
          moves = bar_tic_tac_toe_get_winner_move (state, PLAYER_O);
          if (moves != 0)
            break;
      ...
      move = bar_tic_tac_toe_get_random_move (moves);
    }
...
static gint
bar_tic_tac_toe_get_state (BarTicTacToe *dialog)
{
  gint         state = 0;
  gint         i;
  const gchar *text;
  ...
  bar_return_val_if_fail (BAR_IS_TIC_TAC_TOE (dialog), 0);
...
static void
bar_tic_tac_toe_move (BarTicTacToe *dialog)
{
  gint move;
  gint state;
  gint level;

  bar_return_if_fail (BAR_IS_TIC_TAC_TOE (dialog));

  state = bar_tic_tac_toe_get_state (dialog);
  level = gtk_combo_box_get_active (GTK_COMBO_BOX (dialog->level));
  move = bar_tic_tac_toe_get_move (state, PLAYER_X, level);
  if (move != -1)
    {
...
static void
bar_tic_tac_toe_highlight_winner (BarTicTacToe *dialog,
                                    gint            winner)
{
  PangoAttribute *attr;
  PangoAttrList  *attrs;
...
static void
bar_tic_tac_toe_button_clicked (GtkWidget      *button,
                                  BarTicTacToe *dialog)
{
  GtkWidget *label;
  gint       state;
  ...
  bar_tic_tac_toe_move (dialog);

  state = bar_tic_tac_toe_get_state (dialog);
  winner = bar_tic_tac_toe_has_winner (state);
  if (winner != 0)
    bar_tic_tac_toe_highlight_winner (dialog, winner);
}



static void
bar_tic_tac_toe_new_game (BarTicTacToe *dialog)
{
  ...
  /* 50/50 the bar will start the game */
...
void
bar_tic_tac_toe_show (void)
{
  GtkWidget *dialog;

  dialog = g_object_new (BAR_TYPE_TIC_TAC_TOE, NULL);
